...
/* lucide 0.462 has no brand marks for X or Discord — its `X` is the close
   glyph — so those two are inline.
   Both are solid fills sitting beside 2px-stroke lucide outlines, and their
   raw ink fills more of the 24-unit box than lucide's does (measured: lucide
   18–20 units wide, X 21.6, Discord a full 24), which made Discord read ~20%
   heavier than its neighbours. Each is scaled about its centre to land on
   lucide's ~20-unit ink width so the row has one optical weight. */
const inkScale = (factor: number) =>
  `translate(12 12) scale(${factor}) translate(-12 -12)`;

const XMark = (props: SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" {...props}>
    <g transform={inkScale(0.93)}>
